...
PREFIX = "Liver"
SPLITS = ["train", "test"]  # thực hiện cho cả train và test

for split in SPLITS:
    print(f"\n===== Processing {PREFIX}_{split} =====")

    # File input
    onehot_csv = f"{PREFIX}_{split}_cd40_onehot_candidate.csv"
    esm_csv    = f"{PREFIX}_{split}_cd40_esm_candidate.csv"
    ...
    # Lưu file đầu ra
    output_path = f"{PREFIX}_{split}_cd40_onehot_esm_candidate.csv"
